...
functionText = svg.append('g')
	...
	.append("text").style('text-anchor', 'middle').text("f(x) = 1.0cos(1.0x + 0.0) + 0.0");

function f(x, a, b, c, d) {
	return (parameterVals[0] * Math.cos(parameterVals[1]*x + parameterVals[2]) + parameterVals[3]);
}
...
function updateFunctionText(){
	str = "f(x) = " + parameterVals[0].toFixed(1) + "cos("+parameterVals[1].toFixed(1)+"x";
